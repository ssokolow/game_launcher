...
# TODO: Decide on a name for this tool and rename "src"
from src.util.common import which
...
# TODO: Move this into some kind of data store. It doesn't belong here.
...
class GameEntry(object):
    ...
    # TODO: Decide how to make a corresponding load()
    #       (At the moment, this is just a specialized repr() for testing)
    def dump(self):
        return {
            'name': self.name,
            'icon': self.icon,
...
class InstalledGameEntry(GameEntry):
    ...
    #       (At the moment, this is just a specialized repr() for testing)
    def dump(self):
        data = super(InstalledGameEntry, self).dump()
        data.update({'base_path': self.base_path})
        return data
...
class GameLauncher(GameSubentry):
    ...
    #       (At the moment, this is just a specialized repr() for testing)
    def dump(self):
        data = super(GameLauncher, self).dump()
        data.update({
            'argv': self.argv,
            'path': self.path,
...
class Precedence(enum.IntEnum):
    """An enumeration used for precedence sorting of plugins."""
    highest = 1
    high = 2
    normal = 3
    low = 4
...
class PrioritizedPlugin(IPlugin):
    plugin_type = "<Unknown>"
    backend_name = None
    ...
    @classmethod
    def get_all(cls):
        plugins = [x.plugin_object for x in
            PluginManagerSingleton.get().getPluginsOfCategory(cls.plugin_type)]
        plugins.sort(key=lambda x: x.precedence)
        return plugins

    def __str__(self):
        return "{} ({})".format(self.backend_name, self.plugin_type)

class IGameProvider(PrioritizedPlugin):
    ...
    def get_default_icon(self):
        # TODO: More robust and with an inheritance-based fallback chain.
        return getIconPath(self.default_icon, 128)
...
class IFallbackGameProvider(PrioritizedPlugin):
    """Base class for sub-plugins used by the fallback provider."""
    plugin_type = "Fallback Game Sub-Provider"
    precedence = Precedence.normal
    ...
    def inspect(self, path):
        # Required
        raise NotImplementedError("Override this to extract metadata for the"
                                  "given path.")

# TODO: Decide on a more dynamically extensible method for declaring
# plugin_types to Yapsy (A decorator perhaps?)
PLUGIN_TYPES = [IGameProvider, IFallbackGameProvider]
