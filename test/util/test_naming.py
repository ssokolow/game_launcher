...
from src.util.naming import c, n, filename_to_name
...
def test_camelcase_to_spaces_integration():
    ...
    assert n.camelcase_to_spaces("CamelCase") == "Camel Case"
...
def test_titlecase_up_integration():
    ...
    assert n.titlecase_up("hello") == "Hello"

def test_program_ext_completeness():
    """Test for comprehensiveness of PROGRAM_EXTS-stripping test"""
    missed = [x for x in test_program_exts if x not in c.PROGRAM_EXTS]
    assert not missed, "Extensions not in PROGRAM_EXTS: %s" % missed

    excess = [x for x in c.PROGRAM_EXTS if x not in test_program_exts]
