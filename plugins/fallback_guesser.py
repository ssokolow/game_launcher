...
from src.util.executables import Roles, classify_executable

# TODO: Finish moving icon-identifying code into ...util.icons
from src.util.icons import ICON_EXTS

# TODO: See if I can justify moving the use of this into ...util
from src.util.common import RESOURCE_DIRS_RE
...
class HeuristicFilesystemGameProvider(interfaces.IFallbackGameProvider):
    ...
    @staticmethod
    def find_files(path):
        """os.listdir() wrapper which returns +x files, icons, and subdirs."""
        executables = {}
        icons = []
        subdirs = []
        ...
        # TODO: Also find .desktop files. Even if they only work when
        #       installed, they provide metadata we can scrape for things like
        #       the game's proper title.
    ...
    def inspect(self, path):
        ...
        if len(icons) < 1:
            ...
            for subname in uncase_map:
                if RESOURCE_DIRS_RE.match(subname):
                    ...
                    subfound = self.find_files(os.path.join(path,
                                                            subname_real))
                    if subfound['icons']:
        ...
        if Roles.play in exes:
            ...
            # TODO: bit.trip.runner and Runner 2 have everything in a folder
            #       and only "install" and related files are at the top level.
            #       We'll need to detect and recurse somehow.
            #       (Also, it should be useful to scrape clues from
            #       un-installed .desktop files that are hard-coded to the
            #       install location)
            # TODO: Games like EufloriaHD also express a similar pattern but
            #       with the mojosetup uninstall stuff being the top-level
            #       thing.
            # TODO: Use a more proper solution than list() for working around
            #       .values() and .keys() returning lists in Python 2.x and
            #       views in Python 3.x. (A first() wrapper defined differently
            #       in each?)
        ...
        #       and icon.*, recognizing the significance of names like
        #       uninstall* and install*, demoting names like extract-*,
